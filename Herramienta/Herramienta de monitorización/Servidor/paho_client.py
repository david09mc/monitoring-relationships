    ...
    try :
        ...
        if msg[1] in knownMACs.values() and rssi<90:
            ...
            #Si hay relación de un nodo a otro pero no a la inversa, se modifica el valor RSSI de la relación ya existente.
                ...
                conn.query(modify_relationship(msg[1], msg[0], cercanía))
            else :
                conn.query(modify_relationship(msg[0], msg[1], cercanía))
...
def on_connect(client, userdata, flags, rc): 
    ...
    client.subscribe('David/TFG/Info') 
    client.subscribe('David/TFG/Start')
    '''while(not working):
        print('working...')'''
    
def handle_exit():
    print('Terminando...')
    client.publish('David/TFG/Start','END', qos=1, retain=True)
    time.sleep(2000)
    sys.exit(1)
...
read_students = read_csv('.\datos_contextuales.csv')
students = get_students_data(read_students, knownMACs)
read_teacher = read_csv('.\profesor.csv')
...
client.publish('David/TFG/Start','GO', qos=1, retain=True)
